...
## above is also available at https://github.com/ICGC-TCGA-PanCancer/pcawg-metadata/blob/main/pcawg-published-original-metadata/pcawg_donor_clinical_August2016_v9.tsv
...
## load the data
breast <- fread("~/Dropbox/ICGC/SimonaHelpedVeronicaInMarch2024/gene_tpm_2017-06-05_v8_breast.csv")
ovary  <- fread("~/Dropbox/ICGC/SimonaHelpedVeronicaInMarch2024/gene_tpm_2017-06-05_v8_ovary.csv")
metadata <- fread("~/Dropbox/ICGC/SimonaHelpedVeronicaInMarch2024/GTEx_metadata.txt")
...
filteredRowsMergedEnsemblHavana <- fread("~/Dropbox/ICGC/SimonaHelpedVeronicaInMarch2024/GTEx_only_BRD4_merged_ensemble_havana_transcripts_all_tissues.csv") ## only 2 transcripts
filteredRowsAllProteinCoding    <- fread("~/Dropbox/ICGC/SimonaHelpedVeronicaInMarch2024/GTEx_only_BRD4_protein_coding_transcripts_all_tissues.csv")         ## 5 transcripts
...
breastTranscriptsMergedEnsemblHavana <- filteredRowsMergedEnsemblHavana[, .(transcript_id, gene_id, ..breastSampleIDs)]
...
dt.ta <- as.data.table(filteredRowsAllProteinCodingWithTissueAnnotation)
setnames(dt.ta, "BRD4 Transcript", "BRD4_Transcript")
dt.ta <- dt.ta[transcript_id %in% c("ENST00000263377.6", "ENST00000360016.9")]
g <- ggplot(dt.ta, aes(x = SMTS, y = expression, fill = transcript_id)) +
  ...
  scale_fill_manual(values=c("ENST00000360016.9"="#f1a340", "ENST00000263377.6"="#998ec3")) + 
  labs(y = "Expression (TPM)") +
  scale_y_continuous(expand = c(0, 0)) +
  geom_pwc(aes(group = transcript_id),
           method = "wilcox_test", 
           label = "{p.adj.signif}",
...
# Perform paired Wilcoxon tests for each SMTS condition
stat_results <- dt.ta[,
  {
    # Split data into paired groups
    long_expr <- expression[transcript_id == "ENST00000263377.6"]
    short_expr <- expression[transcript_id == "ENST00000371835.8"]
    
    # Ensure pairing is correct
    if (length(long_expr) == length(short_expr)) {
      test <- wilcox.test(long_expr, short_expr, paired = TRUE)
      list(p_value = test$p.value)  # Store the p-value
    } else {
      list(p_value = NA)  # Handle mismatch in lengths
    }
  },
  by = SMTS
]
stat_results[, p_adj := p.adjust(p_value, method = "fdr")]

ggsave(filename = "~/Dropbox/ICGC/figs/allProteinCodingBRD4IsoformsPlot_FIXED.pdf", 
       plot = g, 
...
#####
## aws s3 ls s3://icgc25k-open/PCAWG/consensus_sv/ --endpoint-url https://object.genomeinformatics.org --no-sign-request
...
brd4_overlap <- GRanges(19, IRanges(15.2e6,15.47e6)) ## hg19 coordinates
...
dcc <- do.call('c',lapply(unique(other.aliquot), function(x) { ##brca.cn$aliquot_id
   ...
   fo <- queryHits(findOverlaps(a1, brd4_overlap))
   a1.overlap <- a1[fo]
...
focal_deletion_cases_ov <- c("e4aaca83-3ae9-47f6-a975-c144767ad705", #OV-AU::AOCS-055
                             ...
                             "1659bae5-3140-4d05-891c-81b48277b2fc", #OV-AU::AOCS-159 ## missed SCNA 
...
focal_deletion_cases_other <- c("f38b5d2e-5cab-45c7-bb0a-38b2efc5c156",
                                "6051f40a-99e5-4461-9255-0e70d757b4d5")
cliff_deletion_cases_breast <- c("fc9ef456-75a2-5967-e040-11ac0c484477")
cliff_deletion_cases_endo <- c("ed32c725-08ae-48eb-8fa2-719b9aeb7550", #UCEC-US::8d9e4917-334b-4c76-aee1-1e22be772db0
                               "cc19a480-9c66-4ee8-a0c0-921acac02689") #UCEC-US::c88e3901-f3ee-436a-ba80-143c8eab7d69
cliff_deletion_cases_ov <- c("80f02aec-c07a-4bcb-b547-e60f8c33a7b1",
                             "bbb2cf2f-8f32-43d4-846c-d1020e6329ec",
                             "d67cad13-e849-48b0-926c-10b6046ba0b9")
...
all_cliff <- c(cliff_deletion_cases_breast, cliff_deletion_cases_endo, cliff_deletion_cases_ov)
all_cases <- c(all_focal, all_cliff)
...
```{r SCNA-Figure-1d}
# Author: Sophie Webster (swebster@broadinstitute.org)
# Contact: Rameen Beroukhim (rameen_beroukhim@dfci.harvard.edu)
# Date last updated: July 17, 2024

# License: GNU GPL2, Copyright (C) 2024 Dana-Farber Cancer Institute

# get SCNA status of BRD4 locus
brd4 <- GRanges(19, IRanges(15.21e6,15.37e6)) ## hg19 coordinates
gr.cn.brd4 <- gr2dt(gr.findoverlaps(gr.cn, brd4_overlap))
gr.cn.brd4[, donor_unique_id := gr.cn$donor_unique_id[query.id]]
gr.cn.brd4[, total_cn := gr.cn$total_cn[query.id]]
gr.cn.brd4[, mean_cn := sum(width * total_cn, na.rm=TRUE) / sum(width), by=donor_unique_id]

## what samples have amplifications
amp_samples_with_focal_del <- intersect(gr.cn.brd4[mean_cn > 2, donor_unique_id], all_focal_donor_id) ## 20/22
amp_samples_without_focal_del <- setdiff(gr.cn.brd4[mean_cn > 2, donor_unique_id], all_focal_donor_id) ## 20/22

amp_del = length(amp_samples_with_focal_del)
unamp_del = 22 - amp_del
amp_undel = length(amp_samples_without_focal_del)
unamp_undel = 2636 - amp_undel

length(amp_samples_with_focal_del) / 22 ## 20/22
length(amp_samples_without_focal_del) / (length(unique(gr.cn$donor_unique_id))-22) ## 734 / 2636

## statistical test
gone_fishing <-
  matrix(c(amp_del, unamp_del, amp_undel, unamp_undel),
         nrow = 2,
         dimnames = list(Amplified = c("Yes", "No"),
                         Deletion = c("Yes", "No")))
tested <- fisher.test(gone_fishing)

# plot fisher's test as bar plot
fishing_fractions <-
  matrix(c(amp_del/(unamp_del+amp_del)*100, unamp_del/(unamp_del+amp_del)*100, amp_undel/(unamp_undel+amp_undel)*100, unamp_undel/(unamp_undel+amp_undel)*100),
         nrow = 2,
         dimnames = list(Amplified = c("Yes", "No"),
                         Deletion = c("Yes", "No")))
...
# Colors for the bars
bar_colors <- c("#b72a26", "#396cb4")

# create dataframe from contingency table
x <- c()
for (row in rownames(gone_fishing)) {
  for (col in colnames(gone_fishing)) {
    x <- rbind(x, matrix(rep(c(row, col), gone_fishing[row, col]), ncol = 2, byrow = TRUE))
  }
}
plot_fish <- as.data.frame(x)
colnames(plot_fish) <- c("Amplified BRD4?", "Deletion in BRD4?")

# show "deletion? yes/no" on the x axis
# i find this more visually appealing but it seems like a less relevant interpretation than having "amp? yes/no" on x
library(ggstatsplot)
p <- ggbarstats(
  plot_fish, `Amplified BRD4?`, `Deletion in BRD4?`,
  results.subtitle = FALSE,
  subtitle = paste0(
    "Fisher's exact test", ", p-value = ",
    round(tested$p.value, 5)
  )
) + theme(text=element_text(size=18)) +
  scale_fill_manual(values=c("#b72a26", "#396cb4"))
#labs(x=expression("Deletion in BRD4?"), fill=expression("Amplified " * italic("BRD4") * "?"))

ggsave(paste0(workdir, "/Sophie/BRD4/results/20240718_fishers_test/", fileDate,"_brd4_amplification_deletion_fishers_bar_plot.pdf"), plot=p)
```
...
## get the average NOTCH3 copy-number
...
focal_plot <- create_beeswarm_plot(dx_combos=list(c("focal","no")), 
                                   ...
                                   x_var="is_focal", y_var="average_cn_notch3", withbox=TRUE) + 
  scale_y_log10(breaks = c(0.3, 1, 3, 10, 30, 100)) + 
scale_fill_manual(values=color_scheme) + ylab("Average NOTCH3 copy number") +
  theme(
...
dtt[is_focal=="focal", median(average_cn_notch3)]
dtt[is_focal=="no", median(average_cn_notch3)]
wilcox.test(dtt[is_focal=="focal", average_cn_notch3], dtt[is_focal=="no" , average_cn_notch3])
ggsave("~/Dropbox/ICGC/figs/NOTCH3_bee_allfocal_UPDATED.pdf", focal_plot, width=2, height=2) 
...
## CCNE1 copy number plot for All Tumors
dttt = dtt[grepl("^(BRCA|OV|UCEC)", donor_unique_id)]
dttt = dtt
focal_plot <- create_beeswarm_plot(dx_combos=list(c("focal","no")), dttt,
...
dttt[is_focal=="focal", median(average_cn_ccne1)]
dttt[is_focal=="no", median(average_cn_ccne1)]
dttt[is_focal=="focal", mean(average_cn_ccne1)]
dttt[is_focal=="no", mean(average_cn_ccne1)]
wilcox.test(dttt[is_focal=="focal", average_cn_ccne1], dttt[is_focal=="no" , average_cn_ccne1])

ggsave("~/Dropbox/ICGC/figs/CCNE1_bee_allfocal_ALLTUMORS_UPDATED.pdf", focal_plot, width=2, height=2) 
